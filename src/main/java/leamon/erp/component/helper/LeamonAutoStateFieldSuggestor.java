...
import leamon.erp.ui.model.DistinctStateCityInfoCellRender;
import leamon.erp.ui.model.StateCityInfoCellRender;


public class LeamonAutoStateFieldSuggestor<T extends List<E>, E> extends JWindow implements KeyListener, FocusListener, ActionListener{
	...
	public LeamonAutoStateFieldSuggestor() {
		lst = new JXList();
		lst.setCellRenderer(new DistinctStateCityInfoCellRender());
	...
	public LeamonAutoStateFieldSuggestor(JTextComponent textArea, JInternalFrame uI) {
		this();
		this.ui  = uI;
		parent =  textArea;
	...
	private void select(boolean enterPressed) {
		if(!this.isVisible()) {
			if(ui instanceof StateCityUI){
				((StateCityUI)ui).getTextFieldStateCode().requestFocus();
			}
			return;
		}
		if(parent == null) {
		...
		parent.setText(info.getState());
		
		/*Releasse 3.6*/
		if(ui instanceof StateCityUI){
			setDistinctStateUI(info);
		}
...
//	private void filter(String text, JList list) {
	private void filter(String text) {
		getPattern(text).ifPresent(pattern -> {
			val.stream()
			.filter(item -> ((Pattern) pattern).matcher((item instanceof StateCityInfo ? (((StateCityInfo) item).getState()) : "")).find() )
	...
	private void setDistinctStateUI(StateCityInfo info){
		((StateCityUI)ui).setDistinctState(info);
	}
}